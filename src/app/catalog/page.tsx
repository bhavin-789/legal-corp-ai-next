        ...
        <div className="container-default px-4">
          <div className="py-[96px]">
            <div className="text-center">
              ...
              {intl.formatMessage({ id: "catalog.hero.title" })}
              </h1>
              <p className="text-xl text-muted-foreground mb-6">
                {intl.formatMessage({ id: "catalog.hero.description" }, { count: user?.plan === "free" ? "3" : "20" })}
              </p>

              {/* User Plan Badge */}
              <div className="flex justify-center items-center space-x-2 mb-6">
                <span className="text-sm text-muted-foreground">
                {intl.formatMessage({ id: "catalog.hero.yourCurrentPlan" })}:
                </span>
                <Badge
                ...
                {user?.plan === "free" && (
                  <Button variant="link" size="sm" asChild>
                    <a href="/pricing">
                    {intl.formatMessage({ id: "catalog.hero.upgradeToPro" })}
                    </a>
                  </Button>
                )}
              </div>
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              <h2 className="text-2xl font-bold">
                {/* {filteredGPTs.length} Asistente
                {filteredGPTs.length !== 1 ? "s" : ""} Legal
                {filteredGPTs.length !== 1 ? "es" : ""} */}
                {intl.formatMessage({ id: "catalog.gptsSection.title" }, { count: filteredGPTs.length })}
              </h2>
        ...
        <section className="py-12 bg-primary/5">
          <div className="container-default px-4">
            <div className="max-w-2xl mx-auto text-center">
              ...
              {intl.formatMessage({ id: "catalog.unlockSuggestionSection.title" })}
              </h3>
              <p className="text-muted-foreground mb-6">
              {intl.formatMessage({ id: "catalog.unlockSuggestionSection.description" })}
              </p>
              <Button size="lg" asChild>
                <a href="/pricing">
                {intl.formatMessage({ id: "catalog.unlockSuggestionSection.seePlans" })}
                </a>
              </Button>
            </div>
          </div>
